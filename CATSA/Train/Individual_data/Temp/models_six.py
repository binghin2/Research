...
def build_model(name: str, W: int = 240, n_channels: int = 1, **kwargs):
    common = dict(seq_len=W, n_channels=n_channels, dropout=0.1)
    if name == "TimeMixerPP":
        return TimeMixerPPClassifier(**common, d_model=128, n_blocks=2, k_periods=3, n_heads=4)
    if name == "Medformer":
        return MedformerClassifier(**common, d_model=128, patch_lens=[4, 8, 16], n_heads=4, n_layers=2)
    if name == "TSLANet":
        return TSLANetClassifier(**common, d_model=128, patch_len=8, patch_stride=4, n_blocks=3)
    if name == "ModernTCN":
        return ModernTCNClassifier(**common, d_model=128, patch_len=8, patch_stride=4, n_blocks=3)
    if name == "CrossGNN":
        return CrossGNNClassifier(**common, d_model=64, patch_len=8, patch_stride=4, n_layers=2)
    if name == "TimesNet":
        return TimesNetClassifier(**common, d_model=64, n_blocks=2, k_periods=3)
    if name == "Mamba2":
        return Mamba2Classifier(**common, d_model=64, d_state=16, n_layers=3)
    if name == "iTransformer":
        return iTransformerClassifier(**common, d_model=128, n_heads=8, n_layers=3, d_ff=256)
    if name == "PatchTST":
        return PatchTSTClassifier(**common, d_model=128, patch_len=16, patch_stride=8, n_heads=8, n_layers=3, d_ff=256)
    if name == "Chronos2":
        pipeline = kwargs.get("pipeline")
        if pipeline is None:
            raise ValueError("Chronos2 requires pipeline= keyword argument")
        return Chronos2Classifier(pipeline, subsample=4, dropout=0.3)
