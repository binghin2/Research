"""
models_ten.py — 11 time-series classifiers for stress detection.
...
class _DualAxisAttn(nn.Module):
    ...
    def forward(self, x):                                   # [B, H, W, D]
        B, H, W, D = x.shape
        h = x.reshape(B * H, W, D)
        if W >= max(self.n_heads, 16):                      # Flash Attention requires seq_len ≥ 16
            h, _ = self.aw(h, h, h, need_weights=False)
        x = self.n1(x + h.reshape(B, H, W, D))
        if H >= max(self.n_heads, 16):                     # Flash Attention requires seq_len ≥ 16
            w = x.permute(0, 2, 1, 3).reshape(B * W, H, D)
            w, _ = self.ah(w, w, w, need_weights=False)
...
class Chronos2Classifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:    # [B, C, T]
        x_sub = x.mean(dim=1)[:, ::self.subsample]         # [B, T//subsample]
